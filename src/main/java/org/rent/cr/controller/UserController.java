...
public class UserController {
    ...
    public Object getPage(@RequestParam(name = "page", defaultValue = "1") Integer page, @RequestParam(name = "per_page", defaultValue = "20") Integer size, @RequestParam(name = "sort", required = false) String field, @RequestParam(name = "filter", required = false) String filter) {
        ...
        Page<User> result = service.getPage(page, size, field, order, filter);
        return result;
    }
    ...
    @JsonView(View.Private.class)
    @PostMapping
    public Object save(@RequestBody User entity) throws NotSavedException, IllegalActionException, NoEntityException {
        entity =  service.save(entity);
        log.info("User added {id=" + entity.getId() + "}");
        return entity;
    }

    @JsonView(View.Private.class)
    @PutMapping("{id}")
    public Object update(@PathVariable("id") User entityFromDb, @RequestBody User entity) {
    ...
    public Object deleteAll() {
        ...
        responseMap.put("message", "User all records deleted {count=" + count + "}");
        log.info("User all records deleted {count=" + count + "}");
        return responseMap;
    }

    @DeleteMapping("{id}")
    public Object delete(@PathVariable("id") User entity) throws NoEntityException {
        service.delete(entity);
        Map<String, String> responseMap = new HashMap<>();
        responseMap.put("message", "User was deleted {id=" + entity.getId() + "}");
